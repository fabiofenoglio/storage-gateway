...
describe('Create child node with content', () => {
  ...
  const defaultNodeTree: NodeTreeSpecification[] = [
    {
      ...
      children: [
        {
          name: 'create-child-node-with-content',
          children: [
            {
              name: 'asd',
  ...
  for (const tenantConfig of tenantConfigurationsUnderTest) {
    it(
      ...
      async () => {
        ...
        const parentNode = nodeTree[0].children![0].children![0].node!;
    ...
    it(tenantConfig.name + ' - should return 201 OK', async () => {
      ...
      expect(resBody.parent).to.eql(parentNode.uuid);
    });
