...
describe('Create node by path from another node', () => {
  ...
  const defaultSubPath = '/subf1/subf2';

  const defaultNodeTree: NodeTreeSpecification[] = [
    {
      name: 'acceptance',
      children: [
        {
          name: 'create-child-node-by-path-with-content',
          children: [
            {
              name: 'subf1',
              children: [
  ...
  const uploadUrl = (tenant: ClientTenant | string, nodeUUID: string) =>
    '/tenant/' +
  ...
  for (const tenantConfig of tenantConfigurationsUnderTest) {
    it(
      ...
      async () => {
        ...
        const nodeTree = await givenNodeTree(
          app,
          defaultTenant,
          defaultNodeTree,
        );
        const parentNode = nodeTree[0].children![0].node!;

        await client
          .post(uploadUrl(defaultTenant, parentNode.uuid))
          .attach(
    ...
    it(
      ...
      async () => {
        ...
        const payload = await payloadBuilder();
        await client
          .post(uploadUrl(otherTenants[0], parentNode.uuid))
          .query({path: defaultSubPath})
          .set(principal.authHeaderName, principal.authHeaderValue)
          .attach(
            payload.attachmentField,
    ...
    it(
      ...
      async () => {
        const payload = await payloadBuilder();
        await client
          .post(uploadUrl('MISSINGTENANT', '00aa-00bb'))
          .query({path: defaultSubPath})
          .set(principal.authHeaderName, principal.authHeaderValue)
          .attach(
            payload.attachmentField,
    ...
    it(
      ...
      async () => {
        ...
        for (const code of malformedCodes) {
          await client
            .post(uploadUrl(code, parentNode.uuid))
            .query({path: defaultSubPath})
            .set(principal.authHeaderName, principal.authHeaderValue)
            .attach(
              payload.attachmentField,
    ...
    it(
      tenantConfig.name + ' - should return 400 when called with bad path',
      async () => {
        const defaultTenant = findTenant(tenantConfig);
        const payload = await payloadBuilder();
    ...
    it(tenantConfig.name + ' - should return 201 OK', async () => {
      const defaultTenant = findTenant(tenantConfig);
      const payload = await payloadBuilder();
      const nodeTree = await givenNodeTree(app, defaultTenant, defaultNodeTree);
      const parentNode = nodeTree[0].children![0].node!;
      ...
      const res = await client
        ...
        .expect('Content-Type', /application\/json/)
        .expect(201);
      const resBody = res.body as CreateNodeResponse;

      // check that it got created in the right location
      const subParentNode =
        nodeTree[0].children![0].children![0].children![0].node!;
      expect(resBody.parent).to.eql(subParentNode.uuid);
    });
    ...
    for (const csType of supportedHashesList) {
      it(
        ...
        async function () {
          ...
          const nodeTree = await givenNodeTree(app, t, defaultNodeTree);
          const parentNode = nodeTree[0].children![0].node!;
          ...
          let res = await client
            .post(uploadUrl(t, parentNode.uuid))
            .query({path: defaultSubPath})
            .set(principal.authHeaderName, principal.authHeaderValue)
            .attach(
              payload.attachmentField,
