...
describe('List children nodes', () => {
  ...
  it('should return 200 OK', async () => {
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FOLDER)!;
    await client
      .get(url(defaultTenant, rootFolder.uuid))
  ...
  it('should return 403 on not-owned tenants', async () => {
    ...
    await client
      .get(url(otherTenants[0], rootFolder.uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(403);
  });
  ...
  it('should return 404 on missing tenants', async () => {
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FOLDER)!;
    await client
      .get(url('MISSINGTENANT', rootFolder.uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(404);
  });
  ...
  it('should have some nodes in folder', async () => {
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FOLDER)!;
    ...
    response.content.forEach(entry => {
      ...
      expect(entry).to.not.have.property('parentId');
      expect(entry).to.not.have.property('parentUuid');
    });
  });

  it('should return 400 when called on file', async () => {
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FILE)!;
    await client
      .get(url(defaultTenant, rootFolder.uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
  ...
  it('should handle pagination with page and size parameters', async () => {
    ...
    const res1 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?page=0&size=' + reqPageSize)
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
    ...
    const res2 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?page=1&size=' + reqPageSize)
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
    ...
    const res3 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?page=2&size=' + reqPageSize)
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
  ...
  it('should handle pagination with only page number', async () => {
    ...
    const res1 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?page=0')
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
    ...
    const res2 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?page=1')
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
  ...
  it('should handle pagination with only page size', async () => {
    ...
    const res1 = await client
      .get(url(defaultTenant, rootFolder.uuid) + '?size=' + reqSize)
      .set(principal.authHeaderName, principal.authHeaderValue)
      .expect('Content-Type', /application\/json/)
      .expect(200);
  ...
  it('should filter by name if specified', async () => {
    // create subfolder
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FOLDER)!;
    const subfolder = await givenFolder(app, rootFolder);

    // create some files
    await givenFile(app, subfolder, {name: 'another-file'});
    await givenFile(app, subfolder, {name: 'filterbyname-file-1'});
    await givenFile(app, subfolder, {name: 'filterbyname-file-2'});
    await givenFile(app, subfolder, {name: 'filterbyname-file-3'});
    await givenFolder(app, subfolder, {name: 'filterbyname-folder-3'});
    await givenFolder(app, subfolder, {name: 'filterbyname-folder-4'});
    ...
    expect(
      (
        (
          await client
            .get(
              url(defaultTenant, subfolder.uuid) +
                '?size=100&filter=' +
                encodeURIComponent(
    ...
    expect(
      (
        (
          await client
            .get(url(defaultTenant, subfolder.uuid) + '?size=100')
            .set(principal.authHeaderName, principal.authHeaderValue)
            .expect(200)
        ).body as ListNodesResponse
      ).totalElements,
