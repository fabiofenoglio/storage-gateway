...
describe('Delete node by path from another node', () => {
  ...
  const defaultSubpath = '/delete-sub-nodes-by-child-path/default-folder';
  ...
  before('setupApplication', async () => {
    ...
    const node2 = await givenFolder(app, defaultStartingNode, {
      name: 'delete-sub-nodes-by-child-path',
    });
  ...
  it('should return 404 on missing path', async () => {
    await client
      .delete(url(defaultTenant, defaultStartingNode.uuid))
      .query({
