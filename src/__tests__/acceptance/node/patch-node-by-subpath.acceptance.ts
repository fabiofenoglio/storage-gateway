...
describe('Patch node by path from another node', () => {
  ...
  const defaultSubpath = '/patch-sub-nodes-by-child-path/default-folder';
  ...
  before('setupApplication', async () => {
    ...
    const node2 = await givenFolder(app, defaultStartingNode, {
      name: 'patch-sub-nodes-by-child-path',
    });
  ...
  it('should return 200 OK', async () => {
    ...
    const res = await client
      .patch(url(defaultTenant, defaultStartingNode.uuid))
      .query({
