...
describe('Update node by path from another node', () => {
  ...
  const defaultSubpath = '/update-sub-nodes-by-child-path/default-folder';
  ...
  before('setupApplication', async () => {
    ...
    const node2 = await givenFolder(app, defaultStartingNode, {
      name: 'update-sub-nodes-by-child-path',
    });
  ...
  it('should return 200 OK', async () => {
    ...
    const res = await client
      .put(url(defaultTenant, defaultStartingNode.uuid))
      .query({
        path: defaultSubpath + '/file-01-01',
      })
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/);
  ...
  it('should return 404 on missing path', async () => {
    ...
    await client
      .put(url(defaultTenant, defaultStartingNode.uuid))
      .query({
        path: defaultSubpath + '/missingpath',
      })
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/)
  ...
  it('should act on the correct node', async () => {
    ...
    const res = await client
      .put(url(defaultTenant, defaultStartingNode.uuid))
      .query({
        path: defaultSubpath + '/file-02-01',
      })
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/);
