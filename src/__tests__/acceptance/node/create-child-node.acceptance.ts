...
describe('Create child node', () => {
  ...
  it('should return 201 OK', async () => {
    ...
    await client
      .post(url(defaultTenant, rootFolder.uuid))
  ...
  it('should return 404 when called on a missing tenant', async () => {
    const rootFolder = rootNodes.find(o => o.type === StorageNodeType.FOLDER)!;
    await client
      .post(url('MISSINGTENANT', rootFolder.uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send({
        ...defaultPayload,
        name: 'file-' + uuidv4(),
  ...
  it(`should return 400 with bad input data`, async () => {
    ...
    for (const propEntry of entries) {
      ...
      const fail = await client
        .post(url(inMemoryTenants[0], rootFolder.uuid))
