...
describe('List nodes by path from another node', () => {
  ...
  const defaultSubpath = '/list-sub-nodes-by-child-path/default-folder';
  ...
  before('setupApplication', async () => {
    ...
    const node2 = await givenFolder(app, defaultStartingNode, {
      name: 'list-sub-nodes-by-child-path',
    });
  ...
  it('should filter by name if specified', async () => {
    ...
    expect(
      (
        (
          await client
            .get(
              url(defaultTenant, defaultStartingNode.uuid) +
                '?size=100&filter=' +
                encodeURIComponent(
    ...
    expect(
      (
        (
          await client
            .get(url(defaultTenant, defaultStartingNode.uuid) + '?size=100')
            .query({
              path: defaultSubpath + '/fold1-02',
            })
            .set(principal.authHeaderName, principal.authHeaderValue)
