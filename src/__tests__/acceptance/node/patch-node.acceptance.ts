...
describe('Update node', () => {
  ...
  it('should return 200 OK', async () => {
    ...
    const res = await client
      .patch(url(defaultTenant, rootNodes[0].uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/);
  ...
  it('should return 403 on not-owned tenants', async () => {
    ...
    await client
      .patch(url(otherTenants[0], otherNodes[0].uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/)
      .expect(403);
  ...
  it('should return 404 on missing tenants', async () => {
    ...
    await client
      .patch(url('MISSINGTENANT', rootNodes[0].uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/)
      .expect(404);
  ...
  it('should return 404 on missing uuid', async () => {
    ...
    await client
      .patch(url(defaultTenant, 'missinguuid'))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/)
      .expect(404);
  ...
  it('should return all properties and hide privates', async () => {
    ...
    const res = await client
      .patch(url(defaultTenant, existing.uuid))
      .set(principal.authHeaderName, principal.authHeaderValue)
      .send(payload)
      .expect('Content-Type', /application\/json/)
      .expect(200);
  ...
  it('should not update data not passed', async () => {
    const existing = await fetch(defaultTenant, rootNodes[0].uuid);
    ...
    expect(response.metadata).to.deepEqual(existing.metadata);
    expect(updated.metadata).to.deepEqual(existing.metadata);
  });

  it('should update, insert but not delete metadata', async () => {
    const existing = await fetch(defaultTenant, rootNodes[0].uuid);
    ...
    expect(response2.metadata.length).to.equal(payload.metadata.length + 1);
    expect(updated2.metadata.length).to.equal(payload.metadata.length + 1);
    expect(
    ...
    response2.metadata.forEach((m: any) =>
      expect(
        updated2.metadata.find((m2: any) => m2.key === m.key)!.value,
      ).to.deepEqual(m.value),
    );
    payload.metadata.forEach((m: any) =>
      expect(
    ...
    expect(response3.metadata.length).to.equal(4);
    expect(updated3.metadata.length).to.equal(4);
    ...
    const response4 = res4.body as UpdateNodeResponse;
    const updated4 = await fetch(defaultTenant, rootNodes[0].uuid);

    expect(response4.metadata.length).to.equal(4);
    expect(updated4.metadata.length).to.equal(4);
  });
});
