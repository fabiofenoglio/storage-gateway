...
describe('Patch node by path from root', () => {
  ...
  const defaultPath =
    '/acceptance/patch-root-nodes-by-child-path/default-folder';
  ...
  before('setupApplication', async () => {
    ...
    const node2 = await givenFolder(app, node1, {
      name: 'patch-root-nodes-by-child-path',
    });
